...
the `TeleopRobotBackend` contract in `src/handumi/real/base.py`:
setup, connect, disconnect, read, home, move_home, write, hold, and health
check. Register it lazily in `make_real_backend`
(`src/handumi/real/registry.py`), declare it in the robot YAML as
