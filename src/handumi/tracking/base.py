...
class ControllerPairSample:
    ...
    def tracking_frame(self) -> dict[str, np.ndarray]:
        return {
            ...
            "observation.tracking.left_tracked": np.array([int(self.left_tracked)], dtype=np.int64),
            "observation.tracking.right_tracked": np.array([int(self.right_tracked)], dtype=np.int64),
            "observation.tracking.device_time_ns": np.array([int(self.device_time_ns)], dtype=np.int64),
            "observation.tracking.pc_monotonic_ns": np.array([int(self.pc_monotonic_ns)], dtype=np.int64),
        }
...
def apply_tcp_calibration_pose7(
    ...
    right = pose_mul(as_pose7(right_controller_pose), calibration.right).astype(np.float32)
